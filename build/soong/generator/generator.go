...
func init() {
	android.RegisterModuleType("cerberus_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = cerberusExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = cerberusExpandVariables(ctx, dep_file)
	...
	cmd := cerberusExpandVariables(ctx, String(g.properties.Cmd))
